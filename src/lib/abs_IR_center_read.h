...
task abs_IR_center_read()
{
	...
	else //IR_180
	{
		...
		abs_dlog(__FILE__ ,"IR","", g_bearing_ac1);
		while(abs(abs_get_angle_sensor_val(RAW_TU)-beginning_val)<g_IR_center_goal_dist_180){}
		if(g_center_goal_pos==1)
		{
			switch(g_bearing_ac2)
			{
			case 2: g_center_goal_pos = 2 break;
			case 0: g_center_goal_pos = 1; break;
			default: g_center_goal_pos = 0; break;
			}
			abs_dlog(__FILE__ ,"IR","", g_bearing_ac2);
		}
	}
