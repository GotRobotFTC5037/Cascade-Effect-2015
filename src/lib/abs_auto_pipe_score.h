...
task abs_auto_pipe_score()
{
	while(true)
	{
		...
		if(nMotorEncoder(shoulder)<g_auto_pipe_score_angle-0&&nMotorEncoder(lift1)>g_auto_pipe_score_hight-800) motor[shoulder] = (((((g_auto_pipe_score_angle-1600)-nMotorEncoder(shoulder))*100)/g_shoulder_max)+g_shoulder_min_speed);
		else if(nMotorEncoder(shoulder)>=g_auto_pipe_score_angle-1600)
		{
			motor[shoulder] = 0;
			shoulder_done = true;
		}
